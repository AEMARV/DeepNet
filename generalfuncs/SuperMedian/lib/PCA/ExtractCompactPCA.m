function [MedianVals,MedianLocs,Bins,xmedrem] = ExtractCompactPCA(x,pivot,calcbin,maxMed)
...
% max med is the maximum number of rows or medians to extract
    MedianLocs = [];
    xmedrem = reshape(x,[],3,1,size(x,4));
    Bins = [];
    ...
    while(dynamic || i<=maxMed)
        meds = extractMedPCA(xmedrem,1,'omitnan');
        if dynamic
        MedianVals = cat(1,MedianVals,meds);
            ...
            locInds = locationSet(curbin,pivot);
