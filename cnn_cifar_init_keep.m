...
j = 1;
net.layers{end+1} = struct('type', 'median','method','instance','weights', {{zeros(1,1,k*32,'single')}});
...
j = j+1;
net.layers{end+1} = struct('type', 'median','method','instance','weights', {{zeros(1,1,k*64,'single')}});
net.layers{end+1} = struct('type', activation,...
    'weights', {{freluinit}}...
    ,'freeze',freeze(j)...
    ,'weightDecay',freluDecay...
