...
if isload
    ...
    freluinit = freluInit(k*32,2, 'single',method);
end
j = 1;

net.layers{end+1} = struct('type', 'fbrelu',...
                          'weights',{{freluinit}},...
                          'freeze',freeze(j),...
...
if isload
    ...
    freluinit = freluInit(k*64,2, 'single',method);
end 
 j = j+1;
net.layers{end+1} = struct('type', 'fbrelu',...
                           'weights', {{freluinit}}...
...
j = j+1;
net.layers{end+1} = struct('type', 'frelu'...
                            ,'weights', {{freluinit}}...
                            ,'freeze',freeze(j),...
                            'weightDecay',~freeze(j)...
                            ,'method',method) ; 
...
net.layers{end+1} = struct('type', 'conv', ... 
 ...
 if isload
    ...
    freluinit = freluInit(10,2, 'single',[method]);
end 
 j = j+1;
net.layers{end+1} = struct('type', 'frelu',...
                            'weights', {{freluinit}}...
...
net.meta.trainOpts.learningRate = [ 0.05*ones(1,100) 0.005*ones(1,50) 0.0005*ones(1,50)] ; 
